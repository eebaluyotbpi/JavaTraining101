	...
	public void createAccount(String accountNumber, String ownerName, double initialBalance)
		...
		if (initialBalance < MINIMUM_INITIAL_BALANCE) {
			...
			// Use String.format with MINIMUM_INITIAL_BALANCE and initialBalance
				throw  new InvalidAccountException(String.format("Minimum Balance: %s : Received : %s", MINIMUM_INITIAL_BALANCE,initialBalance));
		...
		try {
			...
			// Pass exception 'e' as last parameter
				logger.error("Failed to create account due to invalid arguments {}", e.getMessage());
			...
			// Pass exception 'e' as second parameter
				throw  new InvalidAccountException("Account creation failed and original exception", e.getCause());
	...
	public void transfer(String fromAccountNumber, String toAccountNumber, double amount)
		...
		if (toAccount == null) {
			...
			// found: " + toAccountNumber
				throw new AccountNotFoundException("Destinatio account not found" + toAccountNumber);
		...
		if (fromAccount.getBalance() < amount) {
			...
			// Use String.format with fromAccount.getBalance() and amount
				throw new InsufficientFundsException(String.format("Balance : %.2f : Required amount : %.2f",fromAccount.getBalance(), amount));
